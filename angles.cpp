	    ...
	    if(bond_exist[i][j] == 1) {
		    ...
		    if(bond_exist[i][k] == 1) {
		        ...
		        double angle_phi = ((acos(ex_ij * ex_ik +
                        ...
                        double angle_phi_deg = angle_phi * (180/3.14159);
...
void display_bond_angles(int number_atoms) {
	...
	log << bond_angle_v[i][8] << "  ";
        ...
        cout << bond_angle_v[i][10];
	log << bond_angle_v[i][10];
        outputfile << bond_angle_v[i][10];
	cout << endl;
        log << endl;
        outputfile << endl;
    }
        ...
        if(k > i) {
            ...
            for(int l = 0; l < number_atoms; l++) {
		        ...
		        double ez_kl = (unit_vectors(k,l,4));
                        ...
                        double angle_jik = bond_angle_v[ba][9];  //in radians

                        //calculation of bond angle between atoms i, k and l
		        double angle_phi_ikl = ((acos(ex_kl * ex_ik +
                        ...
                        //beginning calculation of dot products 
                        double exx = (ex_ijk * ex_ikl);
                        double eyy = (ey_ijk * ey_ikl);    
                        double ezz = (ez_ijk * ez_ikl);

                        //calculation of torsion angles (tau) with conversion from radians to degrees
                        double tau_ijkl = acos(((exx + eyy + ezz) / 
                                                (sin(angle_jik) * sin(angle_phi_ikl))))
                        		        * (180/3.14159);
        ...
        if(j > i) {
            for(int l = 0; l < number_atoms; l++) {
		        ...
		        double ez_jl = (unit_vectors(j,l,4));
                        ...
                        //calculation of bond angles between atoms i, j and l
                        ...
                        //calculation of torsion angles (tau) with conversion from radians to degrees
                        double tau_ijkl = acos(((exx + eyy + ezz) / 
                                                (sin(angle_jik)*sin(angle_phi_ijl))))
                                                * (180/3.14159);
